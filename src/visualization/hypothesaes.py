...
def plot_hypotheses_bars(
    ...
    *,
    ...
    >>> import polars as pl
    >>> df = pl.DataFrame(
    ...     {"interpretation": ["usa ironia"], "target_separation_score": [0.3]}
    ... )
    ...
    labels = [text[:_LABEL_MAX_CHARS] for text in hypotheses_table[_INTERPRETATION_COLUMN].to_list()]
    ...
    normalizer = plt.Normalize(-np.abs(values).max(), np.abs(values).max())
