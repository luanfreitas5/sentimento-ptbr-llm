"""Classificador de sentimento por CNN (TextCNN, PyTorch) sobre texto tokenizado.

Implementa a Fase 9 (Seção 4.5 do documento mestre): uma camada de embedding
treinada do zero seguida de convoluções 1D em paralelo com múltiplos
tamanhos de filtro (arquitetura TextCNN de Kim, 2014), max-pooling global e
uma cabeça linear de classificação
(``configs/model_params.yaml -> deep_learning.convolutional``).

``torch`` é uma dependência pesada e opcional: o import ocorre de forma
tardia, em :meth:`CNNSentimentClassifier.fit`/:meth:`predict_proba`, para
...
class CNNSentimentClassifier:
    """Classificador de sentimento por TextCNN sobre embeddings treinados do zero.
    ...
    num_filters : int, optional
        Número de filtros de convolução por tamanho em ``filter_sizes``, by
        default 128.
    filter_sizes : Sequence[int], optional
        Tamanhos (larguras, em tokens) dos filtros de convolução aplicados
        em paralelo, by default (3, 4, 5).
    dropout : float, optional
        Taxa de dropout antes da cabeça linear, by default 0.3.
    ...
    max_sequence_length : int, optional
        ...
        deve ser maior ou igual ao maior valor em ``filter_sizes``, by
        default 128.
    ...
    def fit(self, X: Sequence[Sequence[str]], y: Sequence[str]) -> "CNNSentimentClassifier":
        ...
        Examples
        --------
        >>> CNNSentimentClassifier(epochs=1, filter_sizes=(2,)).fit(
        ...
        try:
            ...
            raise ModelError(
                ...
                "para treinar o classificador CNN."
            ) from exception
        ...
        class _TextCNNModule(nn.Module):
            """Embedding treinável + convoluções 1D paralelas + max-pooling + cabeça linear."""
            ...
            def __init__(self) -> None:
                ...
                self.classifier_head = nn.Linear(num_filters * len(filter_sizes), num_classes)
            ...
            def forward(self, token_ids: "torch.Tensor") -> "torch.Tensor":
                ...
                return self.classifier_head(self.dropout_layer(concatenated))

        module = _TextCNNModule()
        ...
        self._module = module
        logger.info(
            "TextCNN treinada: vocabulário de %d termo(s), %d classe(s).",
            vocabulary_size,
            num_classes,
        )
        return self
...
def build_cnn_classifier(**overrides: object) -> CNNSentimentClassifier:
    """Fábrica do classificador TextCNN.

    Parameters
    ----------
    **overrides : object
        Hiperparâmetros que sobrescrevem os padrões do construtor de
        :class:`CNNSentimentClassifier` (espelhando
        ``configs/model_params.yaml -> deep_learning.convolutional``).
    ...
    Examples
    --------
    >>> build_cnn_classifier(num_filters=64).num_filters
    64
    """
    logger.info("Construindo CNNSentimentClassifier (overrides=%s).", overrides)
    return CNNSentimentClassifier(**overrides)  # type: ignore[arg-type]
